...
```{r readData}
dir = 'all_data/data_check_classI_conv'
...
YFP = YFP_files %>% map_dfr(~read_csv(.)) %>%
  ...
  separate(field, into = c("sample","field"), sep = " ") %>% # times all T0
...
rval <- c(rep(NA,10))
names(rval) <- c("r1y1","cy1","cr1","y1y2","y2r1","y2c","y1r2","r1r2","r2c","r2y2")
return(rval)

case_when((is_empty(r[1]) | is_empty(y[1])) ~ RYdf$r1y1 <- NA,
          (is_empty(c) | is_empty(y[1])) ~ RYdf$cy1 <- NA,
          (is_empty(c) | is_empty(r[1])) ~ RYdf$cr1 <- NA,
          (is_empty(y[1]) | is_empty(y[2])) ~ RYdf$y1y2 <- NA,
          (is_empty(y[2]) | is_empty(r[1])) ~ RYdf$y2r1 <- NA,
          (is_empty(c) | is_empty(y[2])) ~ RYdf$y2c <- NA,
          (is_empty(y[1]) | is_empty(r[2])) ~ RYdf$y1r2 <- NA,
          (is_empty(r[1]) | is_empty(r[2])) ~ RYdf$r1r2 <- NA,
          (is_empty(r[2]) | is_empty(c)) ~ RYdf$r2c <- NA,
          (is_empty(r[2]) | is_empty(y[2])) ~ RYdf$r2y2 <- NA,
          (!is_empty(r[1]) | !is_empty(y[1])) ~ RYdf$r1y1 <- min(m[r,y]),
          (!is_empty(c) | !is_empty(y[1])) ~ RYdf$cy1 <- min(m[c,y[1]]),
          (!is_empty(c) | !is_empty(r[1])) ~ RYdf$cr1 <- min(m[c,r[1]]),
          (!is_empty(y[1]) | !is_empty(y[2])) ~ RYdf$y1y2 <- min(m[y[1],y[2]]),
          (!is_empty(y[2]) | !is_empty(r[1])) ~ RYdf$y2r1 <- min(m[y[2],r[1]]),
          (!is_empty(c) | !is_empty(y[2])) ~ RYdf$y2c <- min(m[c,y[2]]),
          (!is_empty(y[1]) | !is_empty(r[2])) ~ RYdf$y1r2 <- min(m[y[1],r[2]]),
          (!is_empty(r[1]) | !is_empty(r[2])) ~ RYdf$r1r2 <- min(m[r[1],r[2]]),
          (!is_empty(r[2]) | !is_empty(c)) ~ RYdf$r2c <- min(m[r[2],c]),
          (is_empty(r[2]) | is_empty(y[2])) ~ RYdf$r2y2 <- min(m[r[2],y[2]]))
rval <- c(r1y1,cy1,cr1, y1y2,y2r1,y2c,y1r2,r1r2,r2c,r2y2)
names(rval) <- c("r1y1","cy1","cr1","y1y2","y2r1","y2c","y1r2","r1r2","r2c","r2y2")
return(rval)
}) %>%
  as_tibble() %>%
  ...
  rename(c("r1y1"=V1,"cy1"=V2,"cr1"=V3,"y1y2"=V4,"y2r1"=V5,"y2c"=V6,"y1r2"=V7,"r1r2"=V8,"r2c"=V9,"r2y2"=V10)) %>%
