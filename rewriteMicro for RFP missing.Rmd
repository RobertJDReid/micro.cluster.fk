...
```{r readData}
dir = '~/Documents/GIT/micro.cluster.fk/data_nolan_conv'
...
full_data = bind_rows(YFP,CFP,CFP_nuclei)
...
```{r clusters}
data_list = full_data %>%
  select(-signal_vol,-signal_intensity,-signal_mean, -surface_area) %>%
...
color_set = c(yfp = "yellow", cfp = "blue", nuc = "grey")
...
#table_two <- final_df %>%
#  filter(signal_code == "cfp nuc rfp") %>%
...
signal_classes %>%
  ...
  scale_color_manual(values=c("deepskyblue","grey","yellow")) +
